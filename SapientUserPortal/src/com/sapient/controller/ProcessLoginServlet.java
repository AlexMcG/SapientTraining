...
public class ProcessLoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		if (session != null) {
			if (uname.equalsIgnoreCase("harshit") && password.equals("abc123")) {
				session.setMaxInactiveInterval(300);
				Cookie cookie = new Cookie("fullname", "Harshit Sanghvi");
				response.addCookie(cookie);
				request.getRequestDispatcher("html\\home.html").forward(
						request, response);
			} else {
